...
# Blackjack, __Due Sept 15th__, by 11PM
...
Try doing a couple of exercises from Chapter 4 in our book, {{ site.book_js }}.
...
* __send me your github username__ (email me at _jversoza at cs dot nyu dot edu_ with subject "CSCI-UA.0480-001 GitHub Username" ... or contact me via NYU classes)
* wait for a notification that states that you've been added to the class's github _organization_
* ...then go to the [class github page](https://github.com/nyu-csci-ua-0480-002-fall-2015)
...
* keep note of the __URL__ of your repository (there's a text box in the middle of the page under __Quick setup — if you’ve done this kind of thing before__)
* this is the repository url... you'll use this when you add your remote repository
* in your project directory, <code>git init</code>... (which creates a local repository)
{% comment %}
...
* Note that __previous versions__ of the homework instructions specify <code>sync-prompt</code> as the library to use. 
* A student informed me that this module did not work on their machine... so as an alternative, use <code>readline-sync</code> below. 
* (If <code>sync-prompt</code> works for you, feel free to use it... as it's compatible with older versions of node).
...
In a file called __range.js__, implement a function called <code>range</code>. It should behave like Python's range function in that the number of arguments it receives to determines how it works. <code>range</code> will produce an <code>Array</code> of <code>Numbers</code> based on the arguments passed into it.
...
When you're done, try using your function with this code to test if it works correctly
...
<pre><code data-trim contenteditable>
// notice the value of arguments before reassignment
// of parameters
function f(a, b) {
...
In a file called __linkedList.js__, implement the functions specified in Exercise 3, "A list", from {{ site.book_js }}
...
In order to write the blackjack program, we can break down the game into several functions:
